...
export default function BulkEditShootersModal({ show, onClose, selectedIds, selectedNames, onSaved }: BulkEditShootersModalProps) {
  ...
  const [result, setResult] = useState<{ updated: number; failed: Array<{ id: string; reason: string }> } | null>(null);
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      const res = await api.bulkUpdateShooters(selectedIds, updates);
      setResult(res);
  ...
  return (
    <Modal show={show} onClose={handleClose} size="lg">
      <ModalHeader>{t('bulkEdit.title', { count: selectedIds.length, entity: t('bulkEdit.shooters') })}</ModalHeader>
      <ModalBody>
        {result ? (
          <div className="space-y-3">
            {result.updated > 0 && (
              <Alert color="success">
                {t('bulkEdit.success', { count: result.updated, entity: t('bulkEdit.shooters') })}
              </Alert>
            )}
          ...
          <div className="space-y-4">
            ...
            <BulkEditFormFields form={form} onChange={setForm} />
          </div>
        )}
      </ModalBody>
      <ModalFooter>
