...
init_env();
var sql = src_default(env.DATABASE_URL);
...
function calcStageParams(scoring_type, paper_targets, steel_targets, no_shoot_targets, hits_per_paper, config) {
  switch (scoring_type) {
    case "comstock":
    case "virginia":
    case "fixed_time":
...
stageRoutes.put("/stages/:id", async (c) => {
  ...
  const [updated] = await sql`
    ...
    SET name = COALESCE(${name}, name),
        ...
        password = ${password !== void 0 ? password : existing.password},
        updated_at = NOW()
    WHERE id = ${id}
    RETURNING *
  `;
...
scoringRoutes.put("/matches/:matchId/stages/:stageId/scores/:registrationId", async (c) => {
  ...
  if (chrono && scoringType === "chrono") {
    ...
    const [match2] = await sql`SELECT organization FROM matches WHERE id = ${matchId}`;
...
var DIVISION_LABELS = {
  ...
  pcc_iron: "PCC Iron",
  limited: "Limited",
  limited_optics: "Limited Optics",
  carry_optics: "Carry Optics",
  ...
  ccp: "CCP",
  bug: "BUG",
  revolver_idpa: "Revolver",
  tactical: "Tactical",
  open_3gun: "Open",
  heavy: "Heavy",
  open_prs: "Open",
  production_prs: "Production",
  any: "Any",
  irons: "Irons",
  open_22: "Open",
...
resultsRoutes.get("/matches/:matchId/results/export/csv", async (c) => {
  const matchId = c.req.param("matchId");
  const [match2] = await sql`SELECT name, date, organization FROM matches WHERE id = ${matchId}`;
  if (!match2) return c.json({ error: "Match not found" }, 404);
  const results = await sql`
    SELECT
      s.first_name, s.last_name,
  ...
  `;
  let csv = "\uFEFF";
  csv += `Match: ${match2.name} (${match2.organization})
Date: ${formatDate(match2.date)}
...
`;
  ...
  return c.text(csv, 200, {
    ...
    "Content-Disposition": `attachment; filename="${match2.name.replace(/[^a-zA-Z0-9]/g, "_")}_results.csv"`
  });
});
resultsRoutes.get("/matches/:matchId/results/export/html", async (c) => {
  ...
  const dq = await sql`
    ...
    ORDER BY s.last_name, s.first_name
  `;
  let html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>${match2.name} Results</title>
    <style>
    ...
    </head><body><h1>${match2.name} (${match2.organization})</h1><p>Date: ${formatDate(match2.date)}</p>`;
  for (const [division, shooters] of Object.entries(divisionGroups)) {
...
function parseCSV(text, hasHeader, columnMapping) {
  if (hasHeader) {
    const records = parse2(text, { columns: true, skip_empty_lines: true, trim: true });
    if (columnMapping) {
      return records.map((row) => {
        ...
        for (const [key, value] of Object.entries(row)) {
          if (!(mapped[key] !== void 0 && Object.values(columnMapping).includes(key))) {
          }
        }
        return mapped;
      });
    ...
    const records = parse2(text, { skip_empty_lines: true, trim: true });
...
function getPgBinDir() {
  ...
  if (resourcesPath) {
    ...
    try {
      ...
      fs2.accessSync(path.join(bundledDir, `psql${ext}`));
  ...
  for (const dir of fallbacks) {
    const ext = process.platform === "win32" ? ".exe" : "";
    try {
      fs2.accessSync(path.join(dir, `psql${ext}`));
      cachedBinDir = dir;
      return dir;
    } catch {
      continue;
