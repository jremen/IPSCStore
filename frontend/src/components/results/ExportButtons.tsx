      ...
      switch (activeTab) {
        case 'byDivision': {
          for (const [division, results] of Object.entries(s.divisionResults).sort(([a], [b]) => a.localeCompare(b))) {
            ...
            lines.push('Position;First Name;Last Name;Division;Category;Power Factor;Match Points');
            for (const r of results as any[]) {
              lines.push(`${r.position};${r.first_name};${r.last_name};${divisionLabel(r.division || '')};${categoryLabel(r.category || '')};${r.power_factor || ''};${Number(r.match_points).toFixed(2)}`);
            }
            lines.push('');
          }
        ...
        case 'overall': {
          lines.push('Position;First Name;Last Name;Division;Category;Power Factor;Match Percent;Match Points');
          for (const r of s.overallResults as any[]) {
            lines.push(`${r.position};${r.first_name};${r.last_name};${divisionLabel(r.division || '')};${categoryLabel(r.category || '')};${r.power_factor || ''};${Number(r.match_percent).toFixed(2)}%;${Number(r.match_points).toFixed(2)}`);
          }
          lines.push('');
          ...
          for (const stage of s.stageResults) {
            ...
            if (stage.divisions && Object.keys(stage.divisions).length > 0) {
              for (const [division, scores] of Object.entries(stage.divisions).sort(([a], [b]) => a.localeCompare(b))) {
                lines.push(divisionLabel(division));
                lines.push('Position;First Name;Last Name;Stage Percent;Stage Points;HF;Net Points;Time');
                for (const r of scores as any[]) {
                  lines.push(`${r.position};${r.first_name};${r.last_name};${Number(r.stage_percent).toFixed(2)}%;${Number(r.stage_points).toFixed(2)};${Number(r.hit_factor).toFixed(4)};${Number(r.net_points).toFixed(2)};${r.time != null ? Number(r.time).toFixed(2) : ''}`);
                }
                lines.push('');
              }
            } else {
              lines.push('Position;First Name;Last Name;Division;Stage Percent;Stage Points;HF;Net Points;Time');
              for (const r of stage.scores || []) {
                lines.push(`${r.position};${r.first_name};${r.last_name};${divisionLabel(r.division || '')};${Number(r.stage_percent).toFixed(2)}%;${Number(r.stage_points).toFixed(2)};${Number(r.hit_factor).toFixed(4)};${Number(r.net_points).toFixed(2)};${r.time != null ? Number(r.time).toFixed(2) : ''}`);
              }
              lines.push('');
            }
...
<table><tr><th>Pos</th><th>Name</th><th>Division</th><th>%</th><th>Points</th></tr>`);
          for (const r of s.overallResults as any[]) {
            parts.push(`<tr><td>${r.position}</td><td>${r.first_name} ${r.last_name}</td><td>${divisionLabel(r.division || '')}</td><td>${Number(r.match_percent).toFixed(2)}%</td><td>${Number(r.match_points).toFixed(2)}</td></tr>`);
          }
          parts.push('</table></div>');
          if (s.dqOverall.length) {
            parts.push(`<div class="section dq-section"><h2>Disqualified</h2>
...
<table><tr><th>Pos</th><th>Name</th><th>%</th><th>Points</th><th>HF</th><th>Net</th><th>Time</th></tr>`);
                for (const r of scores as any[]) {
                  parts.push(`<tr><td>${r.position}</td><td>${r.first_name} ${r.last_name}</td><td>${Number(r.stage_percent).toFixed(2)}%</td><td>${Number(r.stage_points).toFixed(2)}</td><td>${Number(r.hit_factor).toFixed(4)}</td><td>${Number(r.net_points).toFixed(2)}</td><td>${r.time != null ? Number(r.time).toFixed(2) : '—'}</td></tr>`);
                }
                parts.push('</table>');
              }
            } else {
              parts.push(`<table><tr><th>Pos</th><th>Name</th><th>Div</th><th>%</th><th>Points</th><th>HF</th><th>Net</th><th>Time</th></tr>`);
              for (const r of stage.scores || []) {
                parts.push(`<tr><td>${r.position}</td><td>${r.first_name} ${r.last_name}</td><td>${divisionLabel(r.division || '')}</td><td>${Number(r.stage_percent).toFixed(2)}%</td><td>${Number(r.stage_points).toFixed(2)}</td><td>${Number(r.hit_factor).toFixed(4)}</td><td>${Number(r.net_points).toFixed(2)}</td><td>${r.time != null ? Number(r.time).toFixed(2) : '—'}</td></tr>`);
              }
              parts.push('</table>');
            }
            if (stage.dq_scores?.length) {
